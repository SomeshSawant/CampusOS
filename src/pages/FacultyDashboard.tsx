...
import { collection, addDoc, serverTimestamp, getDocs, query, orderBy, limit, onSnapshot, deleteDoc, doc } from 'firebase/firestore';
...
export default function FacultyDashboard() {
  ...
  const handleDeleteNotification = async (notifId: string) => {
    if (!window.confirm('Are you sure you want to delete this transmission? This action cannot be undone.')) return;
  ...
  return (
    <div className="flex h-screen w-full bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white overflow-hidden font-sans transition-colors duration-300">
      ...
      <motion.aside 
        ...
        className="hidden md:flex bg-gray-50 dark:bg-black/40 border-r border-gray-200 dark:border-white/5 backdrop-blur-xl flex-col pt-6 pb-6 shadow-2xl relative z-10 transition-colors"
      >
        <div className={`px-6 mb-10 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#E97262] to-[#c75141] flex items-center justify-center shrink-0">
            ...
            {!isSidebarCollapsed && (
              <motion.div
                ...
                exit={{ opacity: 0, x: -10 }}
              >
                <h1 className="text-xl font-bold tracking-tight whitespace-nowrap">Faculty Portal</h1>
          ...
          <button 
            ...
            className={`p-2 rounded-xl bg-slate-200 dark:bg-white/5 text-slate-500 dark:text-neutral-500 hover:text-rose-500 transition-colors ${isSidebarCollapsed ? 'absolute -right-3 top-20 z-50 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 shadow-xl' : ''}`}
          >
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!isSidebarCollapsed && <span className="text-sm font-bold tracking-tight whitespace-nowrap">{item.label}</span>}
                {isActive && !isSidebarCollapsed && (
                  <motion.div 
                    layoutId="activePill"
                    className="absolute -right-4 w-1 h-6 bg-rose-500 rounded-l-full"
                  />
                )}
              </motion.button>
            );
        ...
        <div className="px-4 mt-auto space-y-3">
          <Link
            to="/"
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 text-sm font-bold text-slate-400 dark:text-neutral-500 hover:text-slate-900 dark:hover:text-neutral-200 hover:bg-slate-100 dark:hover:bg-white/[0.03] ${isSidebarCollapsed ? 'justify-center' : ''}`}
          >
            <Home size={18} />
          ...
          <button
            ...
            className={`w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl transition-all duration-300 text-sm font-bold text-slate-400 dark:text-neutral-500 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 ${isSidebarCollapsed ? 'justify-center' : ''}`}
          >
            <LogOut size={18} />
      ...
      <main className="flex-1 overflow-hidden relative pb-20 md:pb-0">
        ...
        <div className="relative z-10 h-full flex flex-col px-4 md:px-6 lg:px-12 py-6 md:py-10 overflow-y-auto md:overflow-hidden">
          ...
          <header className="flex justify-between items-center mb-8 md:mb-12 shrink-0">
            ...
            >
              <div>
                <h2 className="text-2xl md:text-4xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic">
                  {navItems.find(i => i.id === activeTab)?.label}
                </h2>
                <div className="flex items-center gap-2 mt-1">
                  <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-rose-500 animate-pulse"></div>
                  <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.25em] text-slate-400 dark:text-neutral-500">System Online</p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div 
              ...
              className="flex items-center gap-2 md:gap-4 bg-white dark:bg-neutral-900/50 border border-slate-200 dark:border-neutral-800 p-1 md:p-1.5 rounded-2xl shadow-xl shadow-black/5"
            >
              <div className="px-2 md:px-4 py-1.5 md:py-2 flex items-center gap-2 md:gap-3">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-tr from-rose-500 to-orange-500 flex items-center justify-center text-[10px] font-black text-white shadow-lg">
                  {user?.displayName?.charAt(0) || 'F'}
                </div>
                <div className="hidden sm:block">
                  <p className="text-[10px] md:text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">{user?.displayName?.split(' ')[0] || 'Faculty'}</p>
                </div>
              </div>
            </motion.div>
          </header>
          ...
          <div className="flex-1 min-h-0 relative">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {activeTab === 'post' && (
                  <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 md:gap-8 h-full">
                    {/* Broadcast Form */}
                    <div className="lg:col-span-3 flex flex-col gap-6">
                      <div className="bg-white dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800/60 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl shadow-black/5 flex flex-col min-h-0">
                    ...
                    <div className="lg:col-span-2 space-y-4 md:space-y-6">
                      <div className="bg-rose-500 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] text-white shadow-2xl shadow-rose-500/20 relative overflow-hidden group">
                        <motion.div 
                          animate={{ rotate: 360 }}
                ...
                {activeTab === 'recent' && (
                  <div className="space-y-6 flex flex-col h-full bg-white dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar pb-10">
                      {recentNotifications.length > 0 ? recentNotifications.map((notif: any, i: number) => (
                        <motion.div 
                          ...
                          className="group p-4 md:p-6 bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-neutral-800 rounded-2xl md:rounded-3xl hover:border-rose-500/50 hover:bg-white dark:hover:bg-neutral-800/80 transition-all duration-500"
                        >
                          <div className="flex items-start gap-4 md:gap-6">
                            <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white dark:bg-neutral-800 flex items-center justify-center text-slate-400 group-hover:text-rose-500 transition-colors shadow-sm">
                              <BellRing size={18} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-2">
                                <h4 className="text-base md:text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase leading-none group-hover:text-rose-500 transition-colors truncate pr-2 md:pr-4">{notif.title}</h4>
                                ...
                                <button
                                  ...
                                  className="p-2 text-slate-300 dark:text-neutral-700 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-all ml-2"
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-t border-gray-200 dark:border-white/5 flex items-center justify-around px-4 z-50">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id as Tab)}
              className={`flex flex-col items-center gap-1 transition-all duration-300 ${isActive ? 'text-rose-500 scale-110' : 'text-slate-400'}`}
            >
              <Icon size={20} className={isActive ? 'fill-current/10' : ''} />
              <span className="text-[10px] font-black uppercase tracking-tighter">{item.label}</span>
              {isActive && <div className="w-1 h-1 bg-rose-500 rounded-full mt-1"></div>}
            </button>
          );
        })}
        <button
