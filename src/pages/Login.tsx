...
export default function Login() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Firestore might be offline. Proceeding with student role...", firestoreErr);
      }
      ...
      if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        ...
        setError('An error occurred during log in: ' + err.message);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      let userRole = 'student';
      try {
        // Check if user exists in firestore, if not create a default one
        ...
        if (!userSnap.exists()) {
          ...
          await setDoc(userRef, {
            ...
            role: 'student', // default for now, ideally they'd sign up
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 md:px-6 w-full h-full py-12 md:py-24">
        ...
        <div className="w-full max-w-md mx-auto p-6 md:p-8 bg-white dark:bg-black/40 backdrop-blur-2xl border border-gray-200 dark:border-white/10 rounded-3xl shadow-2xl transition-colors">
          ...
          <p className="text-gray-500 dark:text-white/60 mb-8 text-sm transition-colors">
            Don't have an account? <Link to="/signup" className="text-[#E97262] hover:underline">Sign up</Link>
          </p>
