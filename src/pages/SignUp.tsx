...
export default function SignUp() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    // PU Email Validation: Must be numeric roll number @goa.paruluniversity.ac.in
    const puEmailRegex = /^\d+@goa\.paruluniversity\.ac\.in$/;
    if (!puEmailRegex.test(email)) {
      const roleName = role === 'faculty' ? 'Faculty' : 'Student';
      setError(`Not a PU ${roleName}. Please use your official university email (e.g. 12345@goa.paruluniversity.ac.in)`);
      return;
    }
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (!puEmailRegex.test(googleEmail)) {
        await auth.signOut();
        const roleName = role === 'faculty' ? 'Faculty' : 'Student';
        setError(`Not a PU ${roleName}. Please sign in with your official university account.`);
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="relative min-h-screen w-full bg-white dark:bg-black flex overflow-y-auto transition-colors duration-300">
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 md:px-6 w-full h-full py-12 md:py-24">
        {/* Form Container */}
        <div className="w-full max-w-md mx-auto p-6 md:p-8 bg-white dark:bg-black/40 backdrop-blur-2xl border border-gray-200 dark:border-white/10 rounded-3xl shadow-2xl transition-colors">
          <h2 
            className="text-3xl md:text-5xl text-gray-900 dark:text-white mb-3 tracking-tight transition-colors"
