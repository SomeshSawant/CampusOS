...
import { doc, updateDoc, collection, query, where, orderBy, getDocs, limit, deleteField, increment, onSnapshot } from 'firebase/firestore';
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    if (!userData || !userData.department || (activeTab !== 'feed' && activeTab !== 'history')) return;
    ...
    return () => unsubscribe();
  }, [activeTab, userData?.department, userData?.program]);
  ...
  return (
    <div className="flex h-screen w-full bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white overflow-hidden font-sans transition-colors duration-300">
      ...
      {userData && userData.role === 'student' && !userData.department && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-3xl p-4 overflow-y-auto">
          <motion.div 
            initial={{ opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            className="bg-white dark:bg-black border border-slate-200 dark:border-neutral-900 rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 max-w-lg w-full shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] relative overflow-hidden my-auto"
          >
      ...
      <motion.aside 
        ...
        className="hidden md:flex bg-white dark:bg-black border-r border-slate-200 dark:border-neutral-900 flex-col pt-10 pb-8 transition-all duration-500 relative z-50"
      >
        <div className={`px-6 mb-12 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} group`}>
          <div className="flex items-center gap-4 overflow-hidden">
            ...
            {!isSidebarCollapsed && (
              ...
              >
                <h1 className="text-lg font-black tracking-tighter uppercase italic text-slate-900 dark:text-white leading-none whitespace-nowrap">Quantum Portal</h1>
          ...
          <button 
            ...
            className={`p-2 rounded-xl bg-slate-100 dark:bg-white/5 text-slate-500 dark:text-neutral-500 hover:text-indigo-600 transition-colors ${isSidebarCollapsed ? 'absolute -right-3 top-24 z-50 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 shadow-xl' : ''}`}
          >
              ...
              >
                ...
                {isActive && !isSidebarCollapsed && (
                  <motion.div 
                    ...
                    className="absolute -right-4 w-1 h-6 bg-indigo-600 rounded-l-full"
                  />
                )}
              </motion.button>
            );
        ...
        <div className="relative z-10 h-full flex flex-col px-4 md:px-6 lg:px-12 py-6 md:py-10 overflow-y-auto md:overflow-hidden">
          ...
          <header className="flex justify-between items-center mb-8 md:mb-12 shrink-0">
            ...
            >
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <div className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-indigo-600 animate-pulse"></div>
                  <p className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.25em] text-slate-400 dark:text-neutral-500">Neural Link Active</p>
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="px-2 md:px-4 py-1.5 md:py-2 flex items-center gap-2 md:gap-3">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-500 flex items-center justify-center text-[10px] font-black text-white shadow-lg">
                  {user?.displayName?.charAt(0) || 'S'}
                </div>
                <div className="hidden sm:block">
                  <p className="text-[10px] md:text-xs font-black text-slate-900 dark:text-white uppercase tracking-tight">{user?.displayName?.split(' ')[0] || 'Student'}</p>
                </div>
              </div>
            </motion.div>
          </header>
          ...
          <div className="flex-1 min-h-0 relative">
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === 'feed' && (
                  <div className="flex flex-col h-full bg-white dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar pb-10">
                      {notifications.length > 0 ? notifications.slice(0, 10).map((notif: any, i: number) => (
                        <motion.div 
                          ...
                          animate={{ opacity: 1, x: 0 }}
                          transition={{ delay: i * 0.05 }}
                          className="group p-5 md:p-6 bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-neutral-800 rounded-2xl md:rounded-3xl hover:border-indigo-600/50 hover:bg-white dark:hover:bg-neutral-800/80 transition-all duration-500 relative overflow-hidden"
                        >
                          ...
                          <div className="flex items-start gap-4 md:gap-6">
                            <div className="flex-1 min-w-0">
                              <div className="flex justify-between items-start mb-2">
                                <h4 className="text-base md:text-lg font-black tracking-tight text-slate-900 dark:text-white uppercase italic group-hover:text-indigo-600 transition-colors leading-none truncate pr-2 md:pr-4">{notif.title}</h4>
                ...
                {activeTab === 'timetable' && (
                  <div className="flex flex-col h-full bg-white dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl overflow-hidden">
                  ...
                  <div className="flex flex-col h-full bg-white dark:bg-neutral-900/40 border border-slate-200 dark:border-neutral-800 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl">
                    <div className="flex items-center justify-between shrink-0 mb-6 md:mb-10">
                      <h3 className="text-sm font-black uppercase tracking-[0.2em] text-slate-500 dark:text-neutral-500 flex items-center gap-3">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-t border-gray-200 dark:border-white/5 flex items-center justify-around px-4 z-[60]">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
            ...
            >
              ...
              {isActive && <div className="w-1 h-1 bg-indigo-600 rounded-full mt-1"></div>}
            </button>
          );
        })}
        <button
